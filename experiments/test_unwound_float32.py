#!/usr/bin/env python3
"""
Test Unwound Computation Against Float32 Model
================================================

We found that float16 model has NaN at layer 27.
Our unwound computation (float64) should match float32 model.
...
def main():
    ...
    # Test
    print("\n--- Testing Unwound vs Float32 Model ---")
    ...
    for i in range(n_samples):
        A = np.random.randint(0, tokenizer.vocab_size)
        B = np.random.randint(0, tokenizer.vocab_size)
        ...
        actual_h_normed = rms_norm(actual_h, final_ln)
        ...
        if i < 5:
            print(f"  Sample {i}: actual={actual_token}, pred={pred_token}, cos={cos:.6f}")
    
    print(f"\n  Results ({n_samples} samples):")
    print(f"    Token accuracy: {correct}/{n_samples} = {correct/n_samples*100:.1f}%")
