...
# ================================================================
# v17 CANDIDATES
# Fix A: pc>0.05 zone — add semantic_diverse path for ir<0.20, lv<0.15
#   Rationale: hypernym (ir=0.00, lv=0%, pc=0.051) was getting polar_local
#   Antonyms in this zone have ir≥0.60 → not affected
#
# Fix B: ir≥0.95, t0r≥0.70, sp<0.07 gate — split on pc level
#   Rationale: en_ar (pc=0.159), en_ko (pc=0.114) need factual_local
#   Suffix axes like ish/ist/ism that fire this gate must have lower pc
#   Threshold TBD from diagnostic below
# ================================================================

def v17(pc, lv, ir, sp=0.0, dig=False, t0r=0.0, _pc_thresh_b=None):
    """v17 with Fix A and Fix B. _pc_thresh_b is the pc split for Fix B."""
    thresh_b = _pc_thresh_b if _pc_thresh_b is not None else 0.10
    if dig: return 'semantic_diverse'
    if pc > 0.35: return 'morph_uniform/relational_geom'
    elif pc > 0.30:
        if lv >= 0.80 and sp > 0.07: return 'phonol_scatter'
        ...
        if lv >= 0.50:
            ...
            if t0r >= 0.70 and sp < 0.07:
                # Fix B: split by pc — high-pc = Arabic/Korean (factual), low-pc = ish/ist (scatter)
                if pc > thresh_b: return 'factual_local'
                return 'phonol_scatter'
            if sp < 0.03: return 'phonol_scatter'
            return 'factual_local/translation'
        elif ir >= 0.60:
        ...
        # Fix A: low ir at low pc = globally-valid but locally-inconsistent = semantic_diverse
...
def run_bench(bench, fn, lbl, verbose_zone=None):
    rows = []
...
print("\nDAY 347: v17 — hypernym (Fix A) + Arabic/Korean (Fix B) fixes")
print("="*70)

# --- Phase 0: Diagnostic — feature profile for axes in critical zones ---
print("\nPhase 0: Diagnostic — feature values for axes in fix zones")
print("  (pc=0.05-0.08 zone for Fix A; ir≥0.95+t0r≥0.70+sp<0.07 zone for Fix B)")

diag_bench = ORIG + GEN
feat_cache = {}
for name, train, ho, true in diag_bench:
    ax, valid, pc, sp = axis_spread(train)
    if ax is None or len(valid) < 2: continue
    lv = loo(ax, valid, RELAXED_MASK)
    ir, t0r = irred(ax, ho, RELAXED_MASK)
    feat_cache[name] = (pc, lv, ir, t0r, sp)
    # Report Fix A candidates (pc=0.05-0.08, lv<0.15)
    if 0.05 < pc <= 0.08 and lv < 0.15:
        print("  FIX-A zone: %-12s pc=%.3f lv=%4.0f%% ir=%.2f t0r=%.2f sp=%.3f  true=%s" % (
            name, pc, lv*100, ir, t0r, sp, true))
    # Report Fix B candidates (ir≥0.95, t0r≥0.70, sp<0.07, pc>0.08)
    if pc > 0.08 and ir >= 0.95 and t0r >= 0.70 and sp < 0.07:
        print("  FIX-B zone: %-12s pc=%.3f lv=%4.0f%% ir=%.2f t0r=%.2f sp=%.3f  true=%s" % (
            name, pc, lv*100, ir, t0r, sp, true))

# --- Phase 1: Run v16 and v17 on 59-axis bench ---
print("\nPhase 1: 59-axis bench regression check", flush=True)
o16 = run_bench(ORIG, v16, 'v16-orig')
g16 = run_bench(GEN,  v16, 'v16-gen')
o17 = run_bench(ORIG, v17, 'v17-orig')
g17 = run_bench(GEN,  v17, 'v17-gen')

s16 = sum(r[8] for r in o16+g16); s17 = sum(r[8] for r in o17+g17)
n   = len(o16)+len(g16)
print("  v16: %d/%d = %.0f%%" % (s16, n, 100*s16/n))
print("  v17: %d/%d = %.0f%%" % (s17, n, 100*s17/n))

# Report any changes
changes = [(r16,r17) for r16,r17 in zip(o16+g16, o17+g17) if r16[6]!=r17[6]]
if changes:
    for r16, r17 in changes:
        n_,pc,lv,ir,t0r,sp,p16,true,ok16 = r16; _,_,_,_,_,_,p17,_,ok17 = r17
        tag = 'v17+' if (not ok16 and ok17) else ('v17-' if (ok16 and not ok17) else '~')
        print("  CHANGE %-12s v16=%-22s v17=%-22s true=%-16s %s" % (n_,p16[:22],p17[:22],true,tag))
else:
    print("  No changes on 59-axis bench.")

# --- Phase 2: OOD bench (hypernym, en_ar, en_ko focus) ---
print("\nPhase 2: OOD fix candidates", flush=True)
OOD_FOCUS = [
    ...
    # also check antonym3 not broken by Fix A
    ...
    # and en_pt (close to Fix A zone: pc=0.053)
...
for name, train, ho, true in OOD_FOCUS:
    ax, valid, pc, sp = axis_spread(train)
    ...
    p17_ = v17(pc, lv, ir, sp, dig, t0r)
    ok16 = match(p16_, true); ok17 = match(p17_, true)
    tag = ('✓→✓' if (ok16 and ok17) else ('✗→✓' if (not ok16 and ok17) else ('✓→✗' if (ok16 and not ok17) else '✗→✗')))
    print("  %-12s pc=%.3f sp=%.3f ir=%.2f lv=%4.0f%% t0r=%.2f  v16=%-22s v17=%-22s true=%-16s %s" % (
        name, pc, sp, ir, lv*100, t0r, p16_[:22], p17_[:22], true, tag))

# --- Phase 3: Threshold sweep for Fix B ---
print("\nPhase 3: Fix B pc-threshold sweep for en_ar/en_ko (which axes fire Fix B zone?)")
for name, (pc, lv, ir, t0r, sp) in sorted(feat_cache.items(), key=lambda x: x[1][0]):
    if pc > 0.08 and ir >= 0.95 and t0r >= 0.70 and sp < 0.07:
        print("  %-14s pc=%.3f sp=%.3f ir=%.2f t0r=%.2f  [in Fix-B zone]" % (name, pc, sp, ir, t0r))

print("\nFix B threshold sweep:")
for thresh in [0.08, 0.09, 0.10, 0.11, 0.12, 0.13, 0.14, 0.15]:
    ok = 0; tot = 0
    for name, train, ho, true in ORIG+GEN:
        ax, valid, pc, sp = axis_spread(train)
        if ax is None or len(valid) < 2 or name not in feat_cache: continue
        pc_, lv_, ir_, t0r_, sp_ = feat_cache[name]
        dig = all(tok.decode([sid]).strip().isdigit() for _,_,sid,_ in valid)
        p = v17(pc_, lv_, ir_, sp_, dig, t0r_, _pc_thresh_b=thresh)
        if match(p, true): ok += 1
        tot += 1
    print("  thresh=%.2f → %d/%d = %.0f%%" % (thresh, ok, tot, 100*ok/tot))
