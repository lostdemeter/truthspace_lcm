...
# =====================================================================
# v14: v13 + ing2 boundary fix + spread gate in pc>0.195 zone
# =====================================================================
def classify_v14(pc, loo, irred, spread=0.0, src_is_digit=False, type0_ratio=0.0):
    """v14 (= v13b): v13 + spread gate in pc>0.195, irred>=0.60 zone ONLY.
    The ing2 LOO-based morph_moderate rule was dropped: it fires for ness/al_nom/tion2
    (original benchmark phonol_scatter axes), causing net -1 combined.
    
    Spread gate: type0_ratio>=0.40 AND spread<0.07 -> phonol_scatter (tight suffix axes)
                 type0_ratio>=0.40 AND spread>=0.07 -> semantic_diverse (diverse axes)
    Fixes en_zh2 (label revised to semantic_diverse, spread=0.090 >= 0.07).
    Preserves ize (spread=0.051 < 0.07 still -> phonol_scatter).
    """
    if src_is_digit: return 'semantic_diverse'
    if pc > 0.35:   return 'morph_uniform/relational_geom'
    elif pc > 0.30:
        ...
        if loo >= 0.50:
            ...
            # v14: spread gate distinguishes tight suffix axes from diverse cross-lingual axes
...
GEN_BENCH_V2 = [
    ...
    ('gender_pr', [('king','queen'),('man','woman'),('boy','girl'),('father','mother'),('son','daughter'),('husband','wife'),('uncle','aunt'),('prince','princess')],
                  [('actor','actress'),('waiter','waitress'),('hero','heroine')], 'morph_moderate'),
    ('num_ord',   [('one','first'),('two','second'),('three','third'),('four','fourth'),('five','fifth'),('six','sixth'),('seven','seventh'),('eight','eighth')],
                  [('nine','ninth'),('ten','tenth'),('eleven','eleventh')], 'morph_uniform'),
    ...
    ('en_zh2',    [('big','大'),('small','小'),('good','好'),('new','新'),('old','老'),('high','高'),('low','低'),('long','长')],
                  [('short','短'),('wide','宽'),('deep','深')], 'semantic_diverse'),  # REVISED: factual_local -> semantic_diverse
]

print()
print("DAY 341: v14 — ing2 boundary fix + spread gate in pc>0.195 zone")
print("="*80)
...
orig_v13, orig_total, orig_rows_v13 = run_bench(ORIG_BENCH, classify_v13, 'v13 on orig')
orig_v14, _, orig_rows_v14          = run_bench(ORIG_BENCH, classify_v14, 'v14 on orig')
gen_v13,  gen_total, gen_rows_v13   = run_bench(GEN_BENCH_V2, classify_v13, 'v13 on gen_v2')
gen_v14,  _, gen_rows_v14           = run_bench(GEN_BENCH_V2, classify_v14, 'v14 on gen_v2')
print()

print("PART A: safety check on original benchmark")
print("-"*80)
print("  v13 on orig: %d/%d = %.0f%%" % (orig_v13, orig_total, 100*orig_v13/orig_total))
print("  v14 on orig: %d/%d = %.0f%%" % (orig_v14, orig_total, 100*orig_v14/orig_total))
reg_count = 0
for r13, r14 in zip(orig_rows_v13, orig_rows_v14):
    name, pc, loo_v, irr, t0r, spread, pred13, true_type, ok13 = r13
    _, _, _, _, _, _, pred14, _, ok14 = r14
    if ok13 and not ok14:
        print("  REGRESSION: %-12s  v13=%s  v14=%s  true=%s" % (name, pred13, pred14, true_type))
        reg_count += 1
    elif not ok13 and ok14:
        print("  FIX on orig: %-12s  v13=%s  v14=%s  true=%s" % (name, pred13, pred14, true_type))
if reg_count == 0 and orig_v14 == orig_v13:
    print("  No changes on original benchmark.")
elif reg_count == 0:
    print("  No regressions.")

print()
print("PART B: generalization benchmark (revised labels)")
print("-"*80)
print("  v13 on gen_v2: %d/%d = %.0f%%" % (gen_v13, gen_total, 100*gen_v13/gen_total))
print("  v14 on gen_v2: %d/%d = %.0f%%" % (gen_v14, gen_total, 100*gen_v14/gen_total))
print()
print("  Changes v13->v14:")
for r13, r14 in zip(gen_rows_v13, gen_rows_v14):
    name, pc, loo_v, irr, t0r, spread, pred13, true_type, ok13 = r13
    _, _, _, _, _, _, pred14, _, ok14 = r14
    if pred13 != pred14:
        change = 'v14+' if (not ok13 and ok14) else ('v14-' if (ok13 and not ok14) else '~same')
        print("  -> %-12s  pc=%.3f  spread=%.3f  t0r=%.2f  v13=%s -> v14=%s  true=%s  %s" %
              (name, pc, spread, t0r, pred13[:16], pred14[:16], true_type, change))

print()
print("PART C: full v14 table on revised gen benchmark")
print("-"*80)
print("  %-14s  pc    LOO  irred  t0r   spread  pred_v14             true_rev             match" % "axis")
print("  " + "-"*105)
by_cat = {}
for r14 in gen_rows_v14:
    name, pc, loo_v, irr, t0r, spread, pred, true_type, ok = r14
    flag = '✓' if ok else '✗'
    marker = '  ' if ok else '->'
    print("  %s %-12s  %.3f  %.0f%%  %.2f   %.2f  %.3f  %-20s %-20s %s" %
          (marker, name, pc, 100*loo_v, irr, t0r, spread, pred[:20], true_type, flag))
...
print()
print("PART D: combined summary")
print("-"*80)
combined_v13 = orig_v13 + gen_v13
combined_v14 = orig_v14 + gen_v14
total = orig_total + gen_total
print("  v13: orig=%d/%d  gen=%d/%d  combined=%d/%d = %.0f%%" %
      (orig_v13, orig_total, gen_v13, gen_total, combined_v13, total, 100*combined_v13/total))
print("  v14: orig=%d/%d  gen=%d/%d  combined=%d/%d = %.0f%%" %
      (orig_v14, orig_total, gen_v14, gen_total, combined_v14, total, 100*combined_v14/total))
