#!/usr/bin/env python3
"""
Day 88 — Full Vocabulary Dimensionality Sweep (401 tokens)

Day 87 used only 167 tokens (list was shorter than intended), causing:
  - best_r=0 at all dims (no neighborhood structure from sparse leaves)
  - LOO=0.8912 at 8D vs Day 77 baseline 0.9303

This day: re-run the dimensionality sweep with the full 401-token
vocabulary from Day 78 (same list, same deduplication). Test tries at
8, 10, 12, 14, 16, 20 axes and find the true optimal dimensionality
for the generative lookup task.

PREDICTION:
  - More tokens → denser leaves → best_r will be > 0 again
  - 12D may still be optimal or may be beaten by 8D (Day 77 was 8D best)
  - 16D+ should degrade (too sparse at 3^16+ leaves for 401 tokens)
"""
import json, math
...
OUTPUT_FILE = str(SCRIPT_DIR / "day88_full_vocab_sweep.json")
...
# Full 401-token vocabulary from Day 78
PROBE_TOKENS = [
    # ---- ANIMALS (50) ----
...
PROBE_TOKENS = list(dict.fromkeys(PROBE_TOKENS))   # deduplicate
print(f"Deduplicated vocabulary: {len(PROBE_TOKENS)} tokens")

# Axis inventory ordered by tier (Day 86 ordering by novelty)
ORDERED_AXIS_NAMES = [
...
def compute_t2(pairs):
    diffs = []
    for s1, s2 in pairs:
        h1, _ = run_model(s1); h2, _ = run_model(s2)
...
# ── Extract hidden states and logits for full vocabulary ─────────────────────
print("Extracting h28 + logits for all tokens ...")
...
def build_addresses(axis_subset, hiddens):
    all_projs = []
    for h in hiddens:
        for name in axis_subset:
            all_projs.append(float(np.dot(h, axes[name])))
    max95 = float(np.percentile(np.abs(all_projs), 95))
    if max95 < 1e-6: max95 = 1.0
    addrs = []
    for h in hiddens:
        addr = []
        for name in axis_subset:
            p = float(np.dot(h, axes[name]))
            if p > INV_PHI * max95:      addr.append("H")
            elif p < -INV_PHI * max95:   addr.append("L")
            elif abs(p) < INV_PHI2 * max95: addr.append("U")
            else: addr.append("H" if p > 0 else "L")
...
def loo_lookup(addresses, cosim_mat, max_r):
...
print("=" * 72)
print(f"Dimensionality sweep ({N} tokens)")
print("=" * 72)
print(f"  {'dims':>6}  {'best_r':>7}  {'best_LOO':>10}  {'coverage%':>11}  {'n_leaves':>10}")

for n_dims in DIM_LEVELS:
    axis_subset = ORDERED_AXIS_NAMES[:n_dims]
    addrs = build_addresses(axis_subset, hiddens)
    leaf_counts = Counter(addrs)
    n_leaves   = len(leaf_counts)
    n_sing     = sum(1 for c in leaf_counts.values() if c == 1)
    coverage   = 100 * (N - n_sing) / N

    max_r  = min(5, n_dims // 2)
    loo    = loo_lookup(addrs, cosim_mat, max_r)
    valid  = {r: v for r, v in loo.items() if v is not None}
    best_r = max(valid, key=valid.get) if valid else -1
    best_c = valid[best_r] if valid else float("nan")

    sweep[n_dims] = {
        "axes": axis_subset,
        "n_leaves": n_leaves, "singleton_rate": n_sing / N,
        "coverage_pct": coverage,
        "loo_by_radius": {r: float(v) if v else None for r, v in loo.items()},
        "best_r": best_r, "best_loo_cosim": best_c,
    }
    print(f"  {n_dims:>6}  {best_r:>7}  {best_c:>10.4f}  {coverage:>11.1f}%  {n_leaves:>10}")
...
for r in range(6):
    row = f"  {r:>4}  "
    for d in DIM_LEVELS:
        v = sweep[d]["loo_by_radius"].get(r)
        row += f"  {v:>8.4f}" if v else f"  {'—':>8}"
    print(row)
print()

# ── Compare with Day 77 baseline ──────────────────────────────────────────────
best_dim = max(DIM_LEVELS, key=lambda d: sweep[d]["best_loo_cosim"])
print("=" * 72)
print("Summary")
print("=" * 72)
print(f"""
  Day 77 baseline (8D, 401 tokens, r≤3): 0.9303
  Day 88 best:  {best_dim}D  LOO={sweep[best_dim]['best_loo_cosim']:.4f}  r={sweep[best_dim]['best_r']}

  Coverage vs dimensionality trade-off:
  {'dims':>6}  {'coverage%':>10}  {'best_LOO':>10}
""")
for d in DIM_LEVELS:
    print(f"  {d:>6}  {sweep[d]['coverage_pct']:>10.1f}%  {sweep[d]['best_loo_cosim']:>10.4f}")
