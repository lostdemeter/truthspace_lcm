...
def stem_overlap(src, tgt):
    """Longest common prefix of src and tgt, as fraction of src length.
    Handles e-drop: live->living (liv), write->writing (writ).
    """
    src_l, tgt_l = src.lower(), tgt.lower()
    # try direct prefix
    n = min(len(src_l), len(tgt_l))
    prefix_len = 0
    for i in range(n):
        if src_l[i] == tgt_l[i]: prefix_len += 1
        else: break
    return prefix_len / max(len(src_l), 1)

def compute_stem_overlap_feature(pairs):
    """Mean stem overlap across training pairs."""
    valid_pairs = [(s, t) for s, t in pairs if get_emb(s)[0] is not None and get_emb(t)[0] is not None]
    if not valid_pairs: return 0.0
    return float(np.mean([stem_overlap(s, t) for s, t in valid_pairs]))
...
# =====================================================================
# v15: v14 + stem_overlap feature + targeted ing2 fix
# =====================================================================
def classify_v15(pc, loo, irred, spread=0.0, src_is_digit=False, type0_ratio=0.0,
                 stem_ovlp=0.0):
    """v15: v14 + Feature 7 (stem_overlap) to distinguish ing2 from phonol_scatter.
    Rule: in pc>0.08, loo>=0.50, irred<0.40 branch:
      if stem_ovlp >= 0.75 (target starts with >=75%% of source): morph_moderate
      (live->living: stem_ovlp=3/4=0.75; want->wanting: 4/4=1.0)
      (act->action: stem_ovlp=3/6=0.50; direct->direction: stem_ovlp=6/6=1.0!)
    Note: this is tested empirically first — see PART A diagnostic.
    """
    if src_is_digit: return 'semantic_diverse'
    if pc > 0.35:   return 'morph_uniform/relational_geom'
    elif pc > 0.30:
        ...
        if loo >= 0.50:
            if irred >= 0.40:
                if loo >= 0.70: return 'phonol_scatter'
                return 'semantic_diverse'
            # v15: stem_overlap gate for high-LOO, low-irred (catches ing2)
            # High stem_ovlp = target is just source + suffix (no phonological change)
...
print()
print("DAY 342: Feature 7 (stem_overlap) diagnostic + v15 test")
print("="*80)

print()
print("PART A: stem_overlap diagnostic on key axes (to verify Feature 7 distinguishes ing2 vs tion2/ness/al_nom)")
print("-"*80)
DIAGNOSTIC_AXES = [
    ('ing2',  [('live','living'),('want','wanting'),('need','needing'),('find','finding'),('keep','keeping'),('call','calling'),('feel','feeling'),('turn','turning')], 'morph_moderate'),
    ('tion2', [('invent','invention'),('observe','observation'),('explain','explanation'),('object','objection'),('describe','description'),('destroy','destruction'),('celebrate','celebration'),('compose','composition')], 'phonol_scatter'),
    ('ness',  [('happy','happiness'),('sad','sadness'),('kind','kindness'),('dark','darkness'),('soft','softness'),('weak','weakness'),('good','goodness'),('hard','hardness')], 'phonol_scatter'),
    ('al_nom',[('arrive','arrival'),('propose','proposal'),('approve','approval'),('refuse','refusal'),('remove','removal'),('survive','survival'),('deny','denial'),('dispose','disposal')], 'phonol_scatter'),
    ('tion',  [('act','action'),('direct','direction'),('educate','education'),('create','creation'),('produce','production'),('relate','relation'),('combine','combination'),('apply','application')], 'phonol_scatter'),
    ('ing',   [('go','going'),('take','taking'),('run','running'),('see','seeing'),('give','giving'),('make','making'),('write','writing'),('read','reading')], 'morph_moderate'),
    ('al_rel',[('nation','national'),('region','regional'),('culture','cultural'),('nature','natural'),('person','personal'),('origin','original'),('emotion','emotional'),('tradition','traditional')], 'phonol_scatter'),
    ('ment',  [('achieve','achievement'),('develop','development'),('manage','management'),('govern','government'),('engage','engagement'),('require','requirement'),('move','movement'),('improve','improvement')], 'phonol_scatter'),
    ('less',  [('hope','hopeless'),('fear','fearless'),('care','careless'),('pain','painless'),('end','endless'),('home','homeless'),('harm','harmless'),('power','powerless')], 'phonol_scatter'),
    ('ful',   [('hope','hopeful'),('care','careful'),('fear','fearful'),('use','useful'),('grace','graceful'),('help','helpful'),('faith','faithful'),('joy','joyful')], 'phonol_scatter'),
    ('ance',  [('perform','performance'),('exist','existence'),('enter','entrance'),('resist','resistance'),('accept','acceptance'),('appear','appearance'),('depend','dependence'),('insist','insistence')], 'phonol_scatter'),
    ('plural',[('cat','cats'),('dog','dogs'),('house','houses'),('car','cars'),('tree','trees'),('book','books'),('bird','birds'),('door','doors')], 'morph_moderate'),
    ('3ps',   [('run','runs'),('walk','walks'),('jump','jumps'),('eat','eats'),('read','reads'),('write','writes'),('play','plays'),('work','works')], 'morph_moderate'),
    ('ed_reg',[('walk','walked'),('talk','talked'),('jump','jumped'),('call','called'),('play','played'),('clean','cleaned'),('open','opened'),('start','started')], 'morph_moderate'),
    ('un_neg',[('happy','unhappy'),('clear','unclear'),('fair','unfair'),('likely','unlikely'),('known','unknown'),('safe','unsafe'),('usual','unusual'),('equal','unequal')], 'phonol_scatter'),
    ('ize',   [('modern','modernize'),('local','localize'),('real','realize'),('social','socialize'),('legal','legalize'),('private','privatize'),('organ','organize'),('terror','terrorize')], 'phonol_scatter'),
    ('ous',   [('danger','dangerous'),('poison','poisonous'),('fame','famous'),('nerve','nervous'),('humor','humorous'),('hazard','hazardous'),('glory','glorious'),('courage','courageous')], 'phonol_scatter'),
    ('ish',   [('child','childish'),('self','selfish'),('fool','foolish'),('fever','feverish'),('clown','clownish'),('book','bookish'),('baby','babyish'),('snob','snobbish')], 'phonol_scatter'),
    ('un_verb',[('lock','unlock'),('wrap','unwrap'),('tie','untie'),('fold','unfold'),('pack','unpack'),('dress','undress'),('cover','uncover'),('do','undo')], 'phonol_scatter'),
    ('pre_pfx',[('view','preview'),('heat','preheat'),('pay','prepay'),('cook','precook'),('treat','pretreat'),('warn','prewarn'),('select','preselect'),('test','pretest')], 'phonol_scatter'),
]
print("  %-12s  stem_ovlp  true_label" % "axis")
print("  " + "-"*42)
for name, pairs, true_label in DIAGNOSTIC_AXES:
    sov = compute_stem_overlap_feature(pairs)
    print("  %-12s  %.3f      %s" % (name, sov, true_label))

print()
print("PART B: full feature table for ORIGINAL benchmark (to find ness/al_nom spread/pc)")
print("-"*80)
print("  %-12s  pc     LOO   irred  t0r   spread  stem_ovlp  true_label" % "axis")
print("  " + "-"*80)
for name, train_pairs, holdout_pairs, true_type in ORIG_BENCH:
    ...
    sov = compute_stem_overlap_feature(train_pairs)
    print("  %-12s  %.3f  %.0f%%  %.2f  %.2f  %.3f   %.3f      %s" %
          (name, pc, 100*loo_v, irr, t0r, spread, sov, true_type))

def run_bench(bench, classifier_fn, use_stem=False, label=''):
    ...
    for name, train_pairs, holdout_pairs, true_type in bench:
        ...
        sov = compute_stem_overlap_feature(train_pairs) if use_stem else 0.0
        if use_stem:
            pred = classifier_fn(pc, loo_v, irr, spread, src_is_digit, t0r, sov)
        else:
            pred = classifier_fn(pc, loo_v, irr, spread, src_is_digit, t0r)
        ok = match(pred, true_type)
        if ok: score += 1
        rows.append((name, pc, loo_v, irr, t0r, spread, sov, pred, true_type, ok))
    return score, len(rows), rows

print()
print("PART C: v14 vs v15 on ORIGINAL benchmark")
print("-"*80)
orig_v14, orig_total, orig_rows_v14 = run_bench(ORIG_BENCH, classify_v14, use_stem=False, label='v14 on orig')
orig_v15, _, orig_rows_v15 = run_bench(ORIG_BENCH, classify_v15, use_stem=True, label='v15 on orig')
print("  v14: %d/%d = %.0f%%" % (orig_v14, orig_total, 100*orig_v14/orig_total))
print("  v15: %d/%d = %.0f%%" % (orig_v15, orig_total, 100*orig_v15/orig_total))
for r14, r15 in zip(orig_rows_v14, orig_rows_v15):
    name, pc, loo_v, irr, t0r, spread, sov, pred14, true_type, ok14 = r14
    _, _, _, _, _, _, _, pred15, _, ok15 = r15
    if pred14 != pred15:
        change = 'v15+' if (not ok14 and ok15) else ('v15-' if (ok14 and not ok15) else '~same')
        print("  %s %-12s  pc=%.3f  sov=%.3f  v14=%s -> v15=%s  true=%s  %s" %
              (('  ' if ok15 else '->'), name, pc, sov, pred14[:14], pred15[:14], true_type, change))

print()
print("PART D: v14 vs v15 on GEN benchmark (revised labels)")
print("-"*80)
gen_v14, gen_total, gen_rows_v14 = run_bench(GEN_BENCH_V2, classify_v14, use_stem=False, label='v14 on gen')
gen_v15, _, gen_rows_v15 = run_bench(GEN_BENCH_V2, classify_v15, use_stem=True, label='v15 on gen')
print("  v14: %d/%d = %.0f%%" % (gen_v14, gen_total, 100*gen_v14/gen_total))
print("  v15: %d/%d = %.0f%%" % (gen_v15, gen_total, 100*gen_v15/gen_total))
print()
print("  Changes v14->v15:")
for r14, r15 in zip(gen_rows_v14, gen_rows_v15):
    name, pc, loo_v, irr, t0r, spread, sov, pred14, true_type, ok14 = r14
    _, _, _, _, _, _, _, pred15, _, ok15 = r15
    if pred14 != pred15:
        change = 'v15+' if (not ok14 and ok15) else ('v15-' if (ok14 and not ok15) else '~same')
        print("  -> %-12s  pc=%.3f  sov=%.3f  v14=%s -> v15=%s  true=%s  %s" %
              (name, pc, sov, pred14[:14], pred15[:14], true_type, change))

print()
print("PART E: full v15 table on gen benchmark")
print("-"*80)
print("  %-14s  pc    LOO  irred  t0r  spread  sov    pred_v15             true_rev             match" % "axis")
print("  " + "-"*110)
by_cat = {}
for r15 in gen_rows_v15:
    name, pc, loo_v, irr, t0r, spread, sov, pred, true_type, ok = r15
    flag = '✓' if ok else '✗'
    marker = '  ' if ok else '->'
    print("  %s %-12s  %.3f  %.0f%%  %.2f  %.2f  %.3f  %.3f  %-20s %-20s %s" %
          (marker, name, pc, 100*loo_v, irr, t0r, spread, sov, pred[:20], true_type, flag))
...
print("  v15: orig=%d/%d  gen=%d/%d  combined=%d/%d = %.0f%%" %
      (orig_v15, orig_total, gen_v15, gen_total, orig_v15+gen_v15, total, 100*(orig_v15+gen_v15)/total))
