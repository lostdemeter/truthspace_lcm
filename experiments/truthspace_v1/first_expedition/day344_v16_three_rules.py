...
def source_ids(word):
    ...
    ids = set()
    for p in [' '+word, word, ' '+word[0].upper()+word[1:], word[0].upper()+word[1:]]:
...
def nn_ret(pred_emb, excl_ids, mask):
    ...
    return tok.decode([i]).strip(), float(sims[i])
...
def best_scale(ax, valid, mask):
    ...
    for s in np.linspace(0.02, 6.0, 30):
        c = sum(1 for _,t,sid,_ in valid if nn_ret(W_E[sid]+s*ax, source_ids(tok.decode([sid]).strip()), mask)[0]==t)
...
def loo(ax, valid, mask):
    ...
    for i in range(len(valid)):
        ...
        s,t,sid,_ = valid[i]
        if nn_ret(W_E[sid]+gs*al, source_ids(s), mask)[0]==t: hits+=1
...
def irred(ax, holdout, mask):
    ...
    for sw,tw in holdout:
        ...
        for s in np.linspace(0.02,6.0,60):
            if nn_ret(W_E[sid]+s*ax, source_ids(sw), mask)[0]==tw: found=True; break
        if not found:
            ni+=1
            if tc>1: nt0+=1
...
def v15b(pc, loo, irr, sp=0.0, dig=False, t0r=0.0):
    if dig: return 'semantic_diverse'
    if pc>0.35: return 'morph_uniform/relational_geom'
    elif pc>0.30:
        if loo>=0.80 and sp>0.07: return 'phonol_scatter'
        return 'morph_uniform/relational_geom'
    elif pc>0.195:
        if loo>=0.50: return 'morph_moderate' if irr<0.40 else 'phonol_scatter'
        elif irr<0.30: return 'morph_moderate'
        elif irr>=0.60:
        ...
        if loo>=0.50:
            if irr>=0.40: return 'phonol_scatter' if loo>=0.70 else 'semantic_diverse'
            if loo>=0.70 and irr>0.05 and sp>0.11: return 'morph_moderate'
        ...
        elif loo==0.0 and 0.20<=irr<0.60: return 'phonol_scatter'
        elif loo==0.0 and irr<0.20: return 'semantic_diverse'
        elif 0.0<loo<0.50 and 0.20<=irr<0.60:
            return 'phonol_scatter' if t0r>=0.80 else 'semantic_diverse'
        elif irr<0.20: return 'phonol_scatter-allomorph'
        return 'borderline'
    elif pc>0.05:
        if irr>=0.85 and loo<0.15: return 'translation/factual_local'
        elif loo>0.15 and irr>0.80: return 'polar_local-partial'
        elif loo>0.15: return 'borderline'
        return 'polar_local'
    else:
        return 'polar_local-partial' if loo>0.15 else 'polar_local'

def v16(pc, loo, irr, sp=0.0, dig=False, t0r=0.0):
    if dig: return 'semantic_diverse'
    if pc>0.35: return 'morph_uniform/relational_geom'
    elif pc>0.30:
        if loo>=0.80 and sp>0.07: return 'phonol_scatter'
        ...
        if loo>=0.50:
            ...
            if t0r>=0.70 and sp<0.07: return 'phonol_scatter'
            if sp<0.03: return 'phonol_scatter'          # Rule A: un_verb (tight morph axis)
            return 'factual_local/translation'
        elif irr>=0.60:
            if t0r>=0.40:
                if loo==0.0: return 'semantic_diverse'   # Rule C: er_noun2 (zero-loo agent nouns)
                return 'phonol_scatter'
            return 'semantic_diverse'
        elif loo==0.0 and 0.20<=irr<0.60: return 'phonol_scatter'
        elif loo==0.0 and irr<0.20: return 'semantic_diverse'
        ...
        if irr<0.20: return 'phonol_scatter'             # Rule B: ary (degenerate derivational)
        return 'polar_local-partial' if loo>0.15 else 'polar_local'
...
print("\nDAY 344: v16 — three rules (un_verb, ary, er_noun2)")
print("="*70)
...
print("\nPART A: safety — original bench changes")
print("  v15b: %d/%d  v16: %d/%d" % (o15,ot,o16,ot))
changed=False
for r5,r6 in zip(or15,or16):
    n,pc,lv,ir,t0r,sp,p5,true,ok5=r5; _,_,_,_,_,_,p6,_,ok6=r6
    if p5!=p6:
        changed=True
        tag='v16+' if (not ok5 and ok6) else ('v16-' if (ok5 and not ok6) else '~')
        print("  -> %-12s pc=%.3f sp=%.3f ir=%.2f loo=%.0f%% t0r=%.2f  %s->%s  true=%s  %s"%(n,pc,sp,ir,lv*100,t0r,p5[:12],p6[:12],true,tag))
if not changed: print("  No changes.")

print("\nPART B: generalization bench changes")
print("  v15b: %d/%d  v16: %d/%d" % (g15,gt,g16,gt))
...
print("\nPART C: full v16 gen table")
by_cat={}
for r in gr16:
    n,pc,lv,ir,t0r,sp,p,true,ok=r
    m='  ' if ok else '->'
    print("  %s %-12s pc=%.3f sp=%.3f ir=%.2f loo=%.0f%% t0r=%.2f  %-20s %-20s %s"%(m,n,pc,sp,ir,lv*100,t0r,p[:20],true,'✓' if ok else '✗'))
    if true not in by_cat: by_cat[true]={'ok':0,'t':0,'f':[]}
    by_cat[true]['t']+=1
    if ok: by_cat[true]['ok']+=1
    else: by_cat[true]['f'].append(n)

print("\n  By category:")
for cat in sorted(by_cat):
    d=by_cat[cat]
    print("  %-22s %d/%d=%.0f%%  %s"%(cat,d['ok'],d['t'],100*d['ok']/d['t'] if d['t'] else 0,'FAIL:'+','.join(d['f']) if d['f'] else ''))
