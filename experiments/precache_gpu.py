#!/usr/bin/env python3
"""
GPU-Accelerated Precache System
================================

Optimized version using GPU for faster precaching.

Estimated times:
- CPU: ~52 hours for 152K tokens
- GPU: ~2-4 hours for 152K tokens (overnight feasible)

Usage:
  # Test mode (first 1000 tokens)
  python precache_gpu.py --test
  
  # Full run (all 152K tokens)
  python precache_gpu.py --full
...
def load_checkpoint() -> Dict:
    ...
    if os.path.exists(CHECKPOINT_FILE):
        with open(CHECKPOINT_FILE, 'r') as f:
            return json.load(f)
    return {"last_token_id": -1, "entries": []}
...
def should_skip_token(token_text: str) -> bool:
    ...
    # Skip tokens starting with special characters
    if text[0] in '<>[]{}()':
        return True
...
def generate_for_entity(model, tokenizer, entity_text: str, device: str, n_tokens: int = 6) -> Optional[CacheEntry]:
    """
    Generate response for an entity name.
    
    Key insight: We cache by ENTITY NAME, not by raw token ID.
    This handles multi-token entities and contextual tokenization.
    """
    entity_text = entity_text.strip()
    
    if should_skip_token(entity_text):
        return None
    
    # Build prompt
    prompt = f"The capital of {entity_text} is"
...
def cluster_patterns(entries: List[CacheEntry]) -> Dict[str, int]:
    ...
    pattern_to_id = {pattern: i for i, (pattern, count) in enumerate(sorted_patterns)}
    
    return pattern_to_id


def run_precache_gpu(model, tokenizer, device: str, start_id: int = 0, end_id: int = None,
                     checkpoint_interval: int = 1000, verbose: bool = True):
    """Run precaching on GPU."""
    ...
    print(f"\n{'='*70}")
    print(f"GPU Precaching tokens {start_id} to {end_id} (of {vocab_size})")
    ...
    for token_id in range(start_id, end_id):
        ...
        batch_entries = generate_batch(model, tokenizer, [token_id], device)
        ...
        # Progress update
        if verbose and (token_id - start_id + 1) % 100 == 0:
...
def test_cache(model, tokenizer, device: str, n_test: int = 10):
    """Test the cache."""
...
def main():
    parser = argparse.ArgumentParser(description="GPU Precache")
    parser.add_argument("--test", action="store_true", help="Test mode (first 1000 tokens)")
    ...
    if args.test:
        entries, pattern_to_id = run_precache_gpu(model, tokenizer, device,
